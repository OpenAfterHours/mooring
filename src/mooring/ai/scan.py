...
def scan_context_secrets(workspace: Path, ctx_dir: str, index) -> list[tuple[str, int, str]]:
    """High-confidence secret findings in the team-authored context files
    (``instructions.md`` + the parsed dictionary files)."""
    targets = [workspace / ctx_dir / "instructions.md"]
    targets += [workspace / r.path for r in index.reports if not r.error]
    findings: list[tuple[str, int, str]] = []
    for path in targets:
