...
from mooring import activity, editor, manifest, paths
...
def outbox_target(workspace: Path, rel_posix: str, *, variant: str = "") -> Path:
    """Where ``rel_posix``'s rendered artifact lands: one dated file per notebook per day.

    Public so the scheduled refresh (:mod:`mooring.app.refresh`) puts its artifact exactly
    where an attended Deliver would — one naming scheme, not two.
    ...
    return out_dir / f"{Path(rel_posix).stem}{tag}-{datetime.now():%Y%m%d}.html"
