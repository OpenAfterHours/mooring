...
def test_propose_cell_emits_and_does_not_inject(ws):
    ...
    res = _call(tools["mooring_propose_cell"], _invocation(code="x = 1 + 1", rationale="demo"))
...
def test_propose_cell_preserves_a_mo_sql_body(ws):
    ...
    _call(tools["mooring_propose_cell"], _invocation(code=body))
...
def test_propose_cell_edit_captures_anchor_and_does_not_write(ws):
    ...
    patches = []
    res = _call(
        _edit_tools(ws, patches)["mooring_propose_cell_edit"],
...
def test_propose_cell_edit_out_of_range_errors(ws):
    (ws / "nb.py").write_text(_REAL_NB, "utf-8")
    res = _call(
        _edit_tools(ws, [])["mooring_propose_cell_edit"], _invocation(index=9, code="z = 0")
    )
    assert res.result_type == "error"
...
def test_propose_notebook_rewrite_replaces_all(ws):
    ...
    patches = []
    _call(
        _edit_tools(ws, patches)["mooring_propose_notebook_rewrite"],
...
def test_propose_tools_normalize_returns_in_cell_bodies(ws):
    ...
    _call(
        tools["mooring_propose_notebook_rewrite"],
    ...
    _call(
        tools["mooring_propose_cell_edit"], _invocation(index=0, code="seed = 5\nreturn (seed,)")
    )
    ...
    _call(tools["mooring_propose_cell"], _invocation(code="total = 1\nreturn (total,)"))
...
def test_a_valid_proposal_still_emits_and_reports_success(ws):
    ...
    before = (ws / "nb.py").read_text("utf-8")
    out = specs["mooring_propose_cell"].handler(_invocation(code="y = x * 2", rationale="why"))
    assert not out.is_error
...
def test_a_duplicate_definition_is_refused_and_nothing_is_proposed(ws):
    ...
    before = (ws / "nb.py").read_text("utf-8")
    out = specs["mooring_propose_cell"].handler(_invocation(code="seed = 2"))
    assert out.is_error
...
def test_a_nested_app_cell_body_is_refused(ws):
    ...
    out = specs["mooring_propose_cell"].handler(_invocation(code=body))
...
def test_a_cell_that_does_not_parse_is_refused(ws):
    ...
    proposals = []
    specs = _specs(_real(ws), proposals)
    out = specs["mooring_propose_cell"].handler(_invocation(code="def broken(:\n    pass"))
...
def test_the_edit_path_is_gated(ws):
    ...
    out = specs["mooring_propose_cell_edit"].handler(_invocation(index=1, code="seed = 2"))
...
def test_the_multi_edit_path_is_gated(ws):
    ...
    out = specs["mooring_propose_notebook_edit"].handler(
        _invocation(edits=[{"index": 1, "code": "x = seed + 2"}], appends=["seed = 99"])
    )
    assert out.is_error and "MB002" in out.text
    assert patches == []
...
def test_the_rewrite_path_is_gated(ws):
    ...
    specs = _specs(_real(ws), patches=patches)
    before = (ws / "nb.py").read_text("utf-8")
    out = specs["mooring_propose_notebook_rewrite"].handler(
        _invocation(cells=["total = 1", "total = 2"])
    )
    assert out.is_error and "MB002" in out.text
    assert patches == []
    assert (ws / "nb.py").read_text("utf-8") == before
...
def test_diagnostics_reach_the_model_only_through_the_egress_scrub(ws, monkeypatch):
    ...
    proposals = []
    specs = _specs(_real(ws), proposals)
    out = specs["mooring_propose_cell"].handler(_invocation(code="y = x * 2"))
...
def test_an_unresolved_name_is_a_note_not_a_refusal(ws, monkeypatch):
    ...
    proposals = []
    specs = _specs(_real(ws), proposals)
    out = specs["mooring_propose_cell"].handler(_invocation(code="out = customer_frame.head()"))
    assert not out.is_error
...
def test_repeated_failures_stop_the_model_retrying(ws):
    ...
    for _ in range(3):
        out = specs["mooring_propose_cell"].handler(bad)
    ...
    assert not specs["mooring_propose_cell"].handler(_invocation(code="y = x * 2")).is_error
    assert "MB002" in specs["mooring_propose_cell"].handler(bad).text
...
def test_a_problem_the_notebook_already_had_does_not_refuse_the_proposal(ws):
    ...
    out = specs["mooring_propose_cell"].handler(_invocation(code="note = 'hello'"))
    assert not out.is_error
...
def test_an_nth_definition_of_an_already_duplicated_name_is_not_laundered(ws):
    ...
    proposals = []
    specs = _specs(ws, proposals)
    out = specs["mooring_propose_cell"].handler(_invocation(code="seed = 3"))
    assert out.is_error and "MB002" in out.text
    assert proposals == []
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (lambda: _invocation(deletes=[0]), "delete"),
...
def test_normalising_the_ordinal_does_not_open_a_new_laundering_hole(ws):
    ...
    proposals = []
    specs = _specs(ws, proposals)
    out = specs["mooring_propose_cell"].handler(_invocation(code=nested))
    assert out.is_error and "MOOR002" in out.text
    assert proposals == []
...
def test_an_unchecked_base_does_not_refuse_the_proposal(ws):
    ...
    patches = []
    specs = _specs(ws, patches=patches)
    out = specs["mooring_propose_notebook_edit"].handler(_invocation(deletes=[0]))
    assert not out.is_error
...
def test_a_poisoned_base_check_does_not_refuse_the_proposal(ws, monkeypatch):
    ...
    proposals = []
    specs = _specs(ws, proposals)
    out = specs["mooring_propose_cell"].handler(_invocation(code="note = 2"))
    assert not out.is_error
...
def test_a_new_problem_on_an_already_broken_notebook_is_still_refused(ws):
    ...
    proposals = []
    specs = _specs(ws, proposals)
    out = specs["mooring_propose_cell"].handler(
        _invocation(
            code=(
                "@app.cell\ndef _():\n    a = 1\n    return (a,)\n\n\n"
                "@app.cell\ndef _():\n    b = 2\n    return (b,)"
...
def test_a_notebook_that_cannot_be_parsed_at_all_does_not_crash_the_tool(ws):
    ...
    proposals = []
    specs = _specs(ws, proposals)
    out = specs["mooring_propose_cell"].handler(_invocation(code="x = 1 + 1"))
    assert not out.is_error
...
def test_a_missing_notebook_does_not_crash_the_tool(ws):
    ...
    proposals = []
    specs = _specs(ws, proposals)
    assert not specs["mooring_propose_cell"].handler(_invocation(code="x = 1")).is_error
...
def test_propose_handlers_run_off_the_copilot_event_loop(ws):
    ...
    for name in ["mooring_propose_cell", *EDIT_TOOL_NAMES]:
        assert inspect.iscoroutinefunction(tools[name].handler), name
...
def test_the_copilot_error_channel_carries_the_diagnostics(ws):
    ...
    proposals = []
    res = _call(_tools(_real(ws), proposals)["mooring_propose_cell"], _invocation(code="seed = 2"))
    assert res.result_type == "error"
